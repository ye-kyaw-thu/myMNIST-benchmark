...
class GRUClassifier(nn.Module):
    def __init__(self, input_dim=28, hidden_dim=192, num_layers=2, num_classes=10, 
    ...
    def init_weights(self):
        for name, param in self.gru.named_parameters():
            if 'weight_ih' in name:
                ...
                param.data.fill_(0)
                # GRU typically doesn't need special bias initialization
                # but we can set it similarly to LSTM for consistency
                n = param.size(0)
                param.data[(n // 3):(2 * n // 3)].fill_(1)  # Update gate bias
...
def main():
    parser = argparse.ArgumentParser(description='GRU Baseline for MNIST')
    ...
    parser.add_argument('--output', type=str, default='gru_results', help='Output directory')
